...
export default function ToolCard({ tool, isOwner, onPublish, onUnpublish, onDelete }: ToolCardProps) {
  ...
  return (
    <article className="bg-[#12121a] border border-white/5 rounded-xl p-5 flex flex-col gap-3 transition-colors hover:border-white/10">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-1">
          <h3 className="text-[1.0625rem] font-semibold leading-tight m-0">
            <Link to={`/tools/${tool.id}`} className="text-gray-100 no-underline hover:text-blue-400 hover:underline">
              {tool.name}
            </Link>
          </h3>
      ...
      {tool.tags.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {tool.tags.map((tag) => (
            <span key={tag} className="text-[0.6875rem] font-medium px-1.5 py-0.5 rounded bg-white/5 text-gray-500 border border-white/5">{tag}</span>
          ))}
        </div>
      )}
      <p className="text-sm text-gray-500 m-0 leading-relaxed">{tool.description}</p>
      <StarRating value={tool.average_rating ?? null} count={tool.rating_count ?? 0} size="sm" />
      <div className="flex flex-wrap gap-2 text-[0.8125rem] text-gray-500">
      ...
      <div className="flex flex-wrap gap-2 mt-auto pt-1">
        {tool.github_url && (
          <a href={tool.github_url} target="_blank" rel="noopener noreferrer" className={`${btnBase} bg-white/10 text-gray-300 border border-white/10 hover:bg-white/15`}>GitHub</a>
        )}
        {tool.official_url && (
