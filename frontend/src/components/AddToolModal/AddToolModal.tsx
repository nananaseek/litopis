...
const inputCls = 'w-full px-3 py-2 border border-slate-300 dark:border-white/10 rounded-lg text-sm bg-white dark:bg-[#12121a] text-slate-900 dark:text-gray-100 placeholder:text-slate-400 dark:placeholder:text-gray-500 outline-none focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/20 transition'
...
export default function AddToolModal({ isOpen, onClose, onSave, initialData, editMode }: AddToolModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 animate-[fadeIn_0.15s_ease-out]" onClick={handleClose}>
      <div className="bg-white dark:bg-[#12121a] rounded-xl border border-slate-200 dark:border-white/10 w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 mx-4 animate-[slideUp_0.2s_ease-out]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-slate-900 dark:text-gray-100">{editMode ? 'Редагувати інструмент' : 'Додати інструмент у мій арсенал'}</h2>
          <button type="button" className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-500 dark:text-gray-400 hover:bg-slate-200 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white text-xl transition" onClick={handleClose} aria-label="Закрити">{'\u00D7'}</button>
        </div>
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          {error && <p className="text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">{error}</p>}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="tool-name" className="text-sm font-medium text-slate-600 dark:text-gray-400">Назва *</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="tool-desc" className="text-sm font-medium text-slate-600 dark:text-gray-400">Опис</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="tool-category" className="text-sm font-medium text-slate-600 dark:text-gray-400">Категорія</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="tool-license" className="text-sm font-medium text-slate-600 dark:text-gray-400">Ліцензія</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="tool-tags" className="text-sm font-medium text-slate-600 dark:text-gray-400">Теги</label>
            <div className="flex flex-wrap gap-1.5 p-2 border border-slate-300 dark:border-white/10 rounded-lg bg-slate-50 dark:bg-[#0b0b12] min-h-9 items-center focus-within:border-blue-500/50 focus-within:ring-2 focus-within:ring-blue-500/20">
              {form.tags.map((tag) => (
              ...
              <input
                ...
                className="flex-1 min-w-24 border-none outline-none bg-transparent text-sm text-slate-900 dark:text-gray-100 p-0.5"
              />
            </div>
          </div>
          <div className="flex flex-col gap-1.5">
          ...
          <fieldset className="border border-slate-200 dark:border-white/10 rounded-lg p-4">
            ...
            <div className="flex flex-col gap-3 mt-2">
              <div className="flex flex-col gap-1.5">
                <label htmlFor="tool-github" className="text-sm text-slate-500 dark:text-gray-500">GitHub</label>
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="tool-official" className="text-sm text-slate-500 dark:text-gray-500">Офіційний сайт</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="tool-download" className="text-sm font-medium text-slate-600 dark:text-gray-400">Посилання на завантаження (необов&apos;язково)</label>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" className="px-4 py-2 text-sm font-medium rounded-lg border border-slate-300 dark:border-white/10 text-slate-700 dark:text-gray-300 hover:bg-slate-100 dark:hover:bg-white/5 transition" onClick={handleClose}>Скасувати</button>
