...
const editInput = 'font-inherit text-[0.9375rem] px-2.5 py-1.5 border border-blue-500/50 rounded-lg bg-[#12121a] text-gray-100 outline-none focus:ring-2 focus:ring-blue-500/20 transition'
...
export default function ToolDetailPage() {
  ...
  if (loading) {
    return (
      <div className="max-w-3xl mx-auto">
        <div className="bg-[#12121a] border border-white/5 rounded-xl p-8 flex flex-col gap-4">
          <div className="w-1/2 h-7 rounded-lg bg-white/10 animate-pulse" />
          <div className="w-4/5 h-4 rounded-lg bg-white/10 animate-pulse" />
    ...
    return (
      <div className="max-w-3xl mx-auto">
        <div className="bg-[#12121a] border border-white/5 rounded-xl p-12 text-center">
          ...
          <h2 className="text-lg font-semibold text-gray-100 mb-4">{error || 'Інструмент не знайдено'}</h2>
          <Link to="/arsenal" className="text-blue-400 font-medium no-underline hover:underline">Повернутися до арсеналу</Link>
        </div>
      </div>
    )
  }
  ...
  const headBtn = 'inline-flex items-center gap-1.5 px-3 py-1.5 text-[0.8125rem] font-medium rounded-lg border border-white/10 bg-transparent text-gray-400 no-underline cursor-pointer transition hover:bg-white/5 hover:border-blue-500/30 hover:text-blue-400'

  return (
    <div className="max-w-3xl mx-auto">
      <Link to="/arsenal" className="inline-flex items-center gap-1 text-blue-400 no-underline text-sm font-medium mb-5 hover:underline">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M10 12L6 8l4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
        Назад до арсеналу
      </Link>

      {/* Top card */}
      <div className="bg-[#12121a] border border-white/5 rounded-xl p-6 mb-5">
        <div className="flex items-start justify-between gap-4 mb-3 max-sm:flex-col">
          <div className="flex items-center gap-2.5 flex-wrap min-w-0">
            {editing ? (
              ...
              <>
                ...
                <h1 className="text-xl font-bold text-gray-100 m-0 leading-tight">{tool.name}</h1>
              </>
            )}
          ...
          <div className="flex items-center gap-2 shrink-0 max-sm:self-start">
            ...
            {!editing && tool.github_url && (
              <a href={tool.github_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[0.8125rem] font-medium rounded-lg bg-white/10 border-white/10 text-gray-300 no-underline cursor-pointer transition hover:bg-white/15">
        ...
        {editing ? (
          ...
          <p className="text-[0.9375rem] text-gray-500 leading-relaxed m-0 mb-5">{tool.description || 'Опис відсутній'}</p>
        )}
        ...
        {!editing && (
          <div className="mb-5 pb-5 border-b border-white/5">
            <div className="flex flex-col gap-1">
              <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Рейтинг</span>
              <StarRating
        ...
        {!editing && user && tool.is_published && (
          <div className="mb-5 pb-5 border-b border-white/5">
            <div className="flex flex-col gap-2">
              <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Мій рейтинг</span>
              <p className="text-sm text-gray-500 m-0">Оберіть кількість зірочок від 1 до 5. Можна змінити в будь-який час.</p>
              <StarRating
                ...
                size="md"
              />
              {ratingSubmitting && <span className="text-xs text-gray-500">Збереження...</span>}
            </div>
          </div>
        )}

        {/* Info box */}
        <div className="bg-[#0b0b12] border border-white/5 rounded-xl p-4 flex flex-col gap-3 mb-5">
          {/* Category */}
          <div className="flex flex-col gap-1">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Категорія</span>
            {editing ? (
          ...
          {/* Tags */}
          <div className="flex flex-col gap-1">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Теги</span>
            {editing ? (
              <div className="flex flex-wrap gap-1.5 p-1.5 border border-blue-500/50 rounded-lg bg-[#12121a] min-h-9 items-center max-w-md focus-within:ring-2 focus-within:ring-blue-500/20">
                {d.tags.map((tag) => (
                ...
                <input className="flex-1 min-w-24 border-none outline-none bg-transparent text-[0.8125rem] text-gray-100 p-0.5" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ',') { e.preventDefault(); addTag() } }} placeholder="Додати тег..." />
              </div>
            ) : (
              <div className="flex flex-wrap gap-1">
                {tool.tags.length > 0
                  ? tool.tags.map((tag) => <span key={tag} className="inline-block text-[0.8125rem] font-medium px-2.5 py-0.5 rounded-md bg-white/5 text-gray-400 border border-white/5">{tag}</span>)
                  : <span className="text-[0.8125rem] text-gray-500 italic">Немає тегів</span>
                }
              </div>
            )}
          </div>
          {/* License */}
          <div className="flex flex-col gap-1">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Ліцензія</span>
            {editing ? (
              <input className={editInputSm} value={d.license} onChange={(e) => setDraft({ ...d, license: e.target.value })} placeholder="MIT, GPL, ..." />
            ) : (
              <div className="flex flex-wrap gap-1">
                {tool.license ? <span className="inline-block text-[0.8125rem] font-medium px-2.5 py-0.5 rounded-md bg-white/5 text-gray-400 border border-white/5">{tool.license}</span> : <span className="text-[0.8125rem] text-gray-500 italic">Не вказано</span>}
              </div>
            )}
          </div>
          {/* URLs (edit only) */}
          {editing && (
            <>
              <div className="flex flex-col gap-1">
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">GitHub URL</span>
              ...
              <div className="flex flex-col gap-1">
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Офіційний сайт</span>
              ...
              <div className="flex flex-col gap-1">
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Завантаження</span>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between gap-4 pt-4 border-t border-white/5 max-sm:flex-col max-sm:items-start">
          <span className="inline-flex items-center gap-1 text-[0.8125rem] text-gray-500">
      ...
      {/* About card */}
      <div className="bg-[#12121a] border border-white/5 rounded-xl p-6 pb-0">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <h2 className="text-xl font-bold text-gray-100 m-0 mb-0.5">Про інструмент</h2>
            <p className="text-sm text-gray-500 m-0">Огляд, можливості та документація</p>
          </div>
          {isOwner && !aboutEditing && (
            <button type="button" className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[0.8125rem] font-medium rounded-lg border border-white/10 text-gray-400 hover:bg-white/5 hover:text-blue-400 hover:border-blue-500/30 transition shrink-0" onClick={startAboutEdit}>
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M11.5 1.5l3 3L5 14H2v-3L11.5 1.5z" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>
              Редагувати
            </button>
          )}
        ...
        <div className="py-6">
          {aboutEditing ? (
            ...
            <>
              {tool.about_content && (
                <div className="mb-6 p-4 rounded-xl bg-[#0b0b12] border border-white/5">
                  <div className="text-[0.9375rem] text-gray-300 leading-relaxed whitespace-pre-wrap">{tool.about_content}</div>
                </div>
              )}
            </>
          )}
          ...
          {!aboutEditing && hasReadme && (
            <div className="border border-white/5 rounded-xl overflow-hidden mt-0">
              <div className="flex items-center gap-1.5 px-5 py-2.5 bg-[#0b0b12] border-b border-white/5 text-gray-200">
                ...
                {githubRepo && <span className="ml-auto text-xs text-gray-500 font-mono">{githubRepo}</span>}
                {isOwner && tool.github_url && (
                  <button type="button" className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-md border border-white/10 text-gray-400 hover:bg-white/5 hover:text-blue-400 hover:border-blue-500/30 transition ml-2 disabled:opacity-60 disabled:cursor-not-allowed" onClick={handleRefreshReadme} disabled={refreshing}>
              ...
              <div className="readme-prose readme-prose--dark p-5 bg-[#0b0b12]" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(tool.readme_content!) }} />
            </div>
          )}

          {!aboutEditing && !hasReadme && isOwner && tool.github_url && (
            <div className="text-center py-8 border border-dashed border-white/10 rounded-xl">
              <p className="text-[0.9375rem] text-gray-500 mb-3">README ще не завантажено з GitHub</p>
              <button type="button" className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg border border-white/10 text-gray-400 hover:bg-white/5 hover:text-blue-400 hover:border-blue-500/30 transition disabled:opacity-60 disabled:cursor-not-allowed" onClick={handleRefreshReadme} disabled={refreshing}>
          ...
          {!aboutEditing && !hasReadme && !(isOwner && tool.github_url) && !tool.about_content && (
            <p className="text-[0.9375rem] text-gray-500 text-center py-8 m-0">Документація ще не додана.</p>
          )}
          {!aboutEditing && !hasReadme && !(isOwner && tool.github_url) && tool.about_content && (
            <p className="text-[0.9375rem] text-gray-500 text-center py-4 m-0">README ще не завантажено. Додайте посилання на GitHub та натисніть «Оновити README».</p>
          )}
        </div>
      </div>
    </div>
